...
export function attention({ projectPath, model, exists, stale = false, staleFile = '', tasks = [] }) {
  ...
  /* Подключена лаборатория или нет — знает один признак на весь клиент,
   ...
   * и первым пунктом «лаборатория не подключена». Параметр остаётся ради тех,
   * кто уже знает, что модель есть, но решает не он.
   * Формулировка говорит ровно то, что проверено: ключ выставлен. Отвечает ли
   * лаборатория, выясняется только когда её о чём-нибудь спросят. */
  const hasLab = exists === true || labConnected();
  if (!hasLab) {
    out.push({
      key: 'no-lab', level: 'note',
      title: 'No laboratory key is set on this machine',
   ...
   * лаборатория, — и на машине без ключа гейт одобрения не видел вообще ничего. */
...
/* Что делать дальше, решает очередь на диске, а не лаборатория.
 *
 * Прежде ветка «нет лаборатории» отвечала «Nothing is queued yet» — причём обе
 ...
 * лаборатория, — и выдумывать имя несуществующей процедуры хуже, чем не ветвиться. */

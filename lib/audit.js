...
import { parseTouches } from './read.js';
 ...
 * Intelligence, и разбивку по областям теперь даёт он — а до тех пор счёт ведётся
 ...
 */
function touched(md, area) {
  // Что назвала задача, разбирается ровно той же функцией, что и в очереди
  // (lib/read.js). Здесь стоял свой регексп `^[a-z][a-z0-9-]{1,60}$`: ручка вида
  // gm_261dcdf61e не проходила его из-за подчёркивания, а ручками сегодня и
  // пишется строка Touches:. В append-only запись не попадало ни одного объекта —
  // и разрезы аудита «по объекту» были пусты на всех проектах сразу.
  const ids = parseTouches(md);

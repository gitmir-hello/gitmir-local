...
export async function fetchConnector() {
  ...
  /* Адрес выдачи называет лаборатория, и брать его на веру нельзя: следом мы
   * отправляем по нему Bearer-ключ. Ошибка (или подмена) в её ответе — и ключ
   * уезжает на чужой хост. Поэтому ключ уходит только на origin самой
   * лаборатории, а всё остальное отвергается вслух: «Download failed: fetch
  ...
  if (!src || src.origin !== new URL(lab().home).origin) {
    return { ok: false, error: 'The laboratory pointed the download at '

...
import { lab, key as labKey, PERSONAL_KEY_NEEDED } from './lab.js';

/* Отказ без личного ключа называет, где его взять: /account/access теперь только
 * выбирает место для агента, а личный ключ показывает /account/me. Адрес — от lab(),
 * чтобы стенд с GITMIR_LAB_URL называл свой кабинет. `gitmir lab status` берёт ту же
 * фразу отсюда, чтобы они не разошлись. */
export const personalKeyNeeded = () => PERSONAL_KEY_NEEDED + ' — it is on ' + lab().home + '/account/me';
...
export async function fetchConnector() {
  const k = labKey();
  if (!k) return { ok: false, error: personalKeyNeeded() };
  const dir = home();
  let meta;
  try {
...
export function run({ project, as = '', onLine = (/** @type {string} */ _l) => {} }) {
  ...
  const k = labKey();
  if (!k) return Promise.resolve({ ok: false, error: personalKeyNeeded() });
  const it = installed();

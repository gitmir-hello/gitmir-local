...
export async function fetchConnector() {
  const k = labKey();
  if (!k) return { ok: false, error: 'Connect Intelligence first — the connector is issued with your key.' };
  const dir = home();
  let meta;
  try {
...
export function run({ project, as = '', onLine = (/** @type {string} */ _l) => {} }) {
  ...
  const k = labKey();
  if (!k) return Promise.resolve({ ok: false, error: 'Connect Intelligence first.' });

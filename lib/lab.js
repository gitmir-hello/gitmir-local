...
/**
 * Is this machine pointed at a laboratory?
 *
 * A key in the environment is the only thing that counts. We deliberately do not
 * probe the network to answer this: a slow or offline check would make every
 * screen that merely wants to say "not connected yet" wait for a timeout.
 */
export const connected = () => !!(process.env.GITMIR_LAB_KEY || '').trim();
...
export function needsLab(what = 'This') {
  return {
    ...
    how: [
      ...
      'Sign in (or sign up) there, open MCP access, and copy the key.',
